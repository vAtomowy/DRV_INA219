...
static esp_err_t prv_ina219_write_register(ina219_obj_t* p_obj, uint8_t reg, uint16_t value, TickType_t ticks_to_wait) {
...
static esp_err_t prv_ina219_read_register(ina219_obj_t* p_obj, uint8_t reg, uint16_t* value, TickType_t ticks_to_wait) {
    uint8_t data[2];
...
esp_err_t rf_ina219_init(uint8_t instance_id, ina219_cfg_t* p_cfg) {
...
esp_err_t ina219_read_bus_voltage(uint8_t instance_id, uint16_t* value, TickType_t ticks_to_wait) {
    ...
    esp_err_t err = prv_ina219_read_register(p_ina219_obj[instance_id], INA219_REG_BUS_V, value, ticks_to_wait);
...
esp_err_t ina219_read_shunt_voltage(uint8_t instance_id, uint16_t* value, TickType_t ticks_to_wait) {
    ...
    esp_err_t err = prv_ina219_read_register(p_ina219_obj[instance_id], INA219_REG_SHUNT_V, value, ticks_to_wait);
    xSemaphoreGive(p_ina219_obj[instance_id]->mux);
    return err;
}
...
esp_err_t ina219_configure(uint8_t instance_id, uint16_t config, TickType_t ticks_to_wait) {
    ...
    esp_err_t err = prv_ina219_write_register(p_ina219_obj[instance_id], INA219_REG_CONFIG, config, ticks_to_wait);
    xSemaphoreGive(p_ina219_obj[instance_id]->mux);
    return err;
}
